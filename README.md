...
* Activate the environment using the following command in a terminal window:
  ...
  * Windows: `activate condaenv`
  * Linux, OS X: `source activate condaenv`
...
* Deactivate the environment using the following command in a terminal window:
  ...
  * Windows: `deactivate condaenv`
  * Linux, OS X: `source deactivate condaenv`
...
│   └── testproject       <- Example python package - place shared code in such a package
